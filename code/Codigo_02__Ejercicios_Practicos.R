...
### Estudiaremos la variable de resultado "movilidad social" (d01_01 - d01_02) en las olas 1 a 5
...
## Transformar de Formato Wide a Long (Qué variables son las que se transforman?)
elsoc_selec |> 
  tidyr::pivot_longer(cols = #COMPLETAR,
...
## Convertir los datos a un formato de panel utilizando pdata.frame (Indicar las variables de índice)
elsoc_panel_01 <- plm::pdata.frame(x = elsoc_long, index = c("COMPLETAR", "COMPLETAR"))


## Verificar si el panel está balanceado
plm::is.pbalanced(#COMPLETAR)         # Balance es sobre estructura de dato
...
### INTERPRETACIÓN: Está balanceado el Panel? COMPLETAR


## Verificar si el panel es consecutivo
summary(plm::is.pconsecutive(#COMPLETAR)) # Hay espacios entre las olas?

### INTERPRETACIÓN: Es un panel consecutivo? COMPLETAR
...
elsoc_panel_02 <- panelr::as_panel(data = #COMPLETAR, id = "COMPLETAR", wave = "COMPLETAR")
...
panelr::complete_data(data = #COMPLETAR, 
...
## Imputación de Variables Fijas (Sexo e Indígena)
elsoc_long |>
  dplyr::group_by(idencuesta) |>
  dplyr::mutate(hombre = ifelse(test = wave == 1, yes = #COMPLETAR, no = dplyr::first(na.omit(COMPLETAR))),
                indig  = ifelse(test = wave == 2, yes = #COMPLETAR,  no = dplyr::first(na.omit(COMPLETAR)))) |>
  dplyr::ungroup() |>
...
## Al usar NA Omit como se afecta el Balance del panel?
#COMPLETAR |> na.omit() |> plm::COMPLETAR() # Panel desbalanceado
elsoc_panel_03 |> na.omit() |> plm::punbalancedness() # Los problemas de desbalance son menores

## Transformar en un Panel Balanceado
?plm::make.pbalanced    # Cuál es el tipo de balance adecuado?
elsoc_panel_03 |> na.omit() |> plm::make.pbalanced(balance.type = "COMPLETAR") -> elsoc_panel_df
...
?plm::plm
## Ecuación: movil = B0 + B1*ambicion + B2*trabajo + B3*cambsoc + B4*esfuerzo + B5*habilidades + B6*hombre + B7*edad + 
##                   B8*educ + B9*ocup + B10*indig + B11*irrel + u
m_pooled <- #COMPLETAR
summary(m_pooled)
...
m_fd <- #COMPLETAR, ELIMINANDO EL INTERCEPTO 

plm::has.intercept(m_fd) 
summary(m_fd)

### INTERPRETACIÓN: Cómo se Interpreta un Efecto? COMPLETAR
...
### INTERPRETACIÓN: Qué pasó con el Coeficiente Estimado de hombre?
...
m_re <- #COMPLETAR, usar random.method = "swar"
summary(m_re)
...
### INTERPRETACIÓN: Qué podemos concluir de esta Tabla?
...
### INTERPRETACIÓN: Qué podemos concluir de estos tests?


## Test de Hausman: Usar Efectos Fijos o Aleatorios?
plm::phtest(x = m_within, x2 = m_re)

### INTERPRETACIÓN: Qué podemos concluir de este test?
